...
- **Reset database and start fresh:**
  ...
  _What it does:_ Runs `payload migrate:fresh` to drop/recreate schemas, and deletes `seed/seedState.json` to clear tracking history.

- **Seed the database:**
  ```bash
  npm run db:seed
  ```
  _What it does:_ Initializes Payload, validates/creates a default superadmin user, and seeds fixtures in dependency order.
