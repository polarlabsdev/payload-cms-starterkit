...
const CalendarInputTrigger = React.forwardRef<HTMLDivElement, CalendarInputTriggerProps>(
  ({ value, placeholder = 'Date', className, ...props }, ref) => (
    <div
      ref={ref}
      role="button"
      tabIndex={0}
      className={cn(
        'flex h-9 w-full cursor-pointer items-center justify-between rounded-md border border-border bg-input px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring',
    ...
    >
      {value
        ...
        : placeholder}
      <RiCalendarLine className="h-4 w-4 shrink-0 opacity-50" />
    </div>
  ),
);
CalendarInputTrigger.displayName = 'CalendarInputTrigger';

export { CalendarInputTrigger };
